...
def olakai(event_type: str, event_name: str, params: OlakaiEventParams) -> None:
    ...
    payload = MonitorPayload(
        email=params.email or "anonymous@olakai.ai",
        ...
        metric3=params.metric3,
        metric4=params.metric4,
        metric5=params.metric5,
...
def olakai_report(prompt: str, response: str, options: Dict[str, Any] = None) -> None:
    ...
    params = OlakaiEventParams(
        prompt=prompt,
        response=response,
        email=options.get("email", "anonymous@olakai.ai"),
        chatId=options.get("chatId"),
        task=options.get("task"),
        subTask=options.get("subTask"),
        dim1=options.get("dim1"),
        dim2=options.get("dim2"),
        dim3=options.get("dim3"),
        dim4=options.get("dim4"),
        dim5=options.get("dim5"),
        metric1=options.get("metric1"),
        metric2=options.get("metric2"),
        metric3=options.get("metric3"),
        metric4=options.get("metric4"),
        metric5=options.get("metric5"),
