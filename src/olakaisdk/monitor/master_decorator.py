...
class OlakaiMasterDecorator:
    def __init__(self, func: Optional[Callable] = None, **kwargs):
        ...
        self.monitor_options: MonitorOptions = MonitorOptions(
            chatId=self.chatId, **kwargs
        )
    ...
    def sub_decorator(self, **kwargs):
        ...
        global_params = {
            field.name: getattr(self.monitor_options, field.name)
            for field in fields(MonitorOptions)
        }

        # Supprimer les paramètres None des paramètres globaux
        global_params = {
            k: v for k, v in global_params.items() if v is not None
        }
