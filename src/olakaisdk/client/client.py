...
class OlakaiClient:
    def __init__(self, api_key: str, domain: str = "https://app.olakai.ai", **kwargs):    
        ...
        self.config = SDKConfig(
            apiKey=api_key,
            monitoringUrl=f"{domain}/api/monitoring/prompt" if domain else "https://staging.app.olakai.ai/api/monitoring/prompt",
            controlUrl=f"{domain}/api/control/prompt" if domain else "https://staging.app.olakai.ai/api/control/prompt",
            **kwargs
        )
        ...
        safe_log('info', f"Initialized Olakai SDK client with config: {self.config}")
        ...
        if self.config.enableStorage:
            try:
                ...
                init_queue_manager(QueueDependencies(self.config, send_with_retry))
